...
hh-accounts-browser-err-not-logged-in = В файле есть cookie hh.ru, но сессия не авторизована. Сначала войдите в аккаунт и сохраните storage_state заново.
...
hh-accounts-remote-login = 🖥 Вход на сервере
hh-login-assist-disabled = Вход на сервере отключён. Попросите администратора включить HH_LOGIN_ASSIST_ENABLED (и UI apply + ключ шифрования).
hh-login-assist-not-configured = Нужны HH_UI_APPLY_ENABLED и HH_TOKEN_ENCRYPTION_KEY для входа на сервере.
hh-login-assist-parallel = Уже выполняется другая сессия входа. Дождитесь окончания или отмените её.
hh-login-assist-rate-limited = Дневной лимит входа на сервере исчерпан. Попробуйте завтра.
hh-login-assist-queued = В очереди: запускается браузер на сервере…
hh-login-assist-browser-open = Откройте ссылку и завершите вход в удалённом браузере (пароль и 2FA): { $url }
hh-login-assist-no-viewer = HH_LOGIN_ASSIST_VIEWER_URL не задан. Нужны DISPLAY и headed Chromium (см. docs/HH_LOGIN_ASSIST.md), либо пришлите JSON storage_state с ПК.
hh-login-assist-starting = Запускается вход на сервере… Можно отменить, пока в очереди.
hh-login-assist-saving = Сохраняем сессию…
hh-login-assist-success = Сессия hh.ru сохранена. Можно откликаться из ленты (UI apply).
hh-login-assist-success-different-account = Сессия hh.ru сохранена в совпавший связанный аккаунт. Сессия принадлежала другому профилю HeadHunter, а не выбранному аккаунту.
hh-login-assist-timeout = Время ожидания входа истекло. Повторите или импортируйте JSON.
hh-login-assist-captcha = Обнаружена капча. Войдите вручную на hh.ru и попробуйте позже, либо импортируйте storage_state.
hh-login-assist-error = Ошибка входа на сервере: { $detail }
hh-login-assist-cancelled = Вход отменён.
...
feed-pick-hh-browser-required = Для этой ленты сейчас нужен аккаунт с сохранённой браузерной сессией hh.ru.
feed-pick-hh-api-required = Для этой ленты сейчас нужен OAuth-подключённый аккаунт HeadHunter.
feed-btn-respond-hh = 📨 Откликнуться на hh.ru
feed-respond-loading-resumes = ⏳ Загружаем список резюме с hh.ru (может занять до минуты)…
...
feed-respond-employer-questions = ⚠️ На hh.ru открылись вопросы работодателя по этой вакансии. Завершите отклик на сайте — вакансия отмечена в ленте.
feed-card-employer-questions = Нужны ответы работодателю — завершите на hh.ru
...
hh-ui-negotiations-limit-notice = hh.ru сейчас не принимает отклики: достигнут лимит активных откликов (открытых переговоров). Закройте часть откликов на сайте или дождитесь, когда работодатель архивирует вакансии, затем попробуйте снова.
hh-ui-negotiations-limit-shortage = Остановлено: лимит активных откликов на hh.ru
feed-respond-skipped-already-applied-toast = Пропуск — уже откликались. Следующая вакансия.
feed-respond-skipped-rate-limited-toast = Пропуск — дневной лимит UI. Следующая вакансия.
...
integrate-duties-report-page = Стр. { $current }/{ $total }
...
we-detail-text-truncated = Длинный текст достижений или обязанностей обрезан из‑за лимита Telegram (4096 символов).
...
we-from-text-pick-intro-achievements =
    Выберите место работы — затем отправьте один длинный текст с фактами и заметками; бот сформирует достижения для резюме.
we-from-text-pick-intro-duties =
    Выберите место работы — затем отправьте один длинный текст с фактами и заметками; бот сформулирует обязанности (несовершенный вид).
we-from-text-prompt-achievements =
    Отправьте один сообщение с опорным текстом: заметки, проекты, стек, результаты. По нему будут сформулированы достижения для этой записи.
we-from-text-prompt-duties =
    Отправьте один сообщение с опорным текстом: заметки, задачи, проекты. По нему будут сформулированы обязанности (несовершенный вид) для этой записи.
we-from-text-empty = Отправьте непустой текст (не только пробелы).
we-from-text-too-long = Слишком длинное сообщение. Сократите текст и попробуйте снова.
we-from-text-busy = Сначала отправьте опорный текст или отмените ввод.
we-from-text-wrong-state = Завершите текущий шаг в боте или отмените его, затем повторите.
...
autoparse-parse-mode-prompt = Выберите режим парсинга:
autoparse-parse-mode = Режим парсинга
autoparse-parse-mode-api = API
autoparse-parse-mode-web = Web
autoparse-parse-mode-api-label = API
autoparse-parse-mode-web-label = Web
autoparse-parse-account-ready = [сессия]
autoparse-parse-account-pick = Этот фильтр использует параметр resume, поэтому для web-парсинга нужен аккаунт HH с сохранённой браузерной сессией. Выберите аккаунт:
autoparse-parse-login-required = Для этого web-фильтра нужна авторизация на HH.ru. Войдите сейчас или откройте HH-аккаунты, затем нажмите «Продолжить после входа».
autoparse-parse-login-for-account = Для аккаунта <b>{ $label }</b> нет сохранённой браузерной сессии. Войдите сейчас, затем нажмите «Продолжить после входа».
autoparse-parse-login-now = Войти сейчас
autoparse-parse-continue-after-login = Продолжить после входа
autoparse-parse-open-hh-accounts = Открыть HH-аккаунты
autoparse-parse-login-started = Вход на сервере запущен. После успешного входа нажмите «Продолжить после входа».
autoparse-parse-login-followup = Когда вход завершится, вернитесь сюда и нажмите «Продолжить после входа».
autoparse-parse-no-login-assist = На сервере недоступен удалённый вход. Откройте HH-аккаунты и загрузите storage_state, затем нажмите «Продолжить после входа».
...
autoparse-detail-keyword-check = Проверка ключевых слов
autoparse-detail-parse-mode = Режим парсинга
autoparse-edit-keywords = ✏️ Редактировать ключевые слова
autoparse-edit-keywords-prompt = Введите новые ключевые слова (текущие: { $current }):
autoparse-edit-keywords-saved = ✅ Ключевые слова обновлены.
autoparse-edit-search-url = ✏️ Редактировать ссылку парсинга
autoparse-edit-search-url-prompt = Введите новый URL поиска HH (текущий: { $current }):
autoparse-edit-search-url-saved = ✅ Ссылка обновлена.
autoparse-edit-parse-mode = ✏️ Режим парсинга
autoparse-edit-parse-mode-saved = ✅ Режим парсинга обновлён.
...
autorespond-section-title = Автоответ
autorespond-detail-enabled = Включено
...
autorespond-btn-settings = ⚡ Настройки автоответа…
autorespond-btn-run = 📨 Запустить автоответ
autorespond-rename = ✏️ Переименовать автоответ
autorespond-rename-prompt = Введите новое название автоответа (текущее: { $current }):
autorespond-rename-empty = Название не может быть пустым.
autorespond-rename-saved = ✅ Название автоответа обновлено.
autorespond-enable = ▶️ Включить автоответ
...
autoparse-run-error-no-session = ❌ Web-парсинг не запущен: для этого фильтра нужна сохранённая браузерная сессия HH.ru.
autoparse-run-error-session-expired = ❌ Web-парсинг не запущен: браузерная сессия HH.ru истекла. Войдите снова и повторите.
...
autoparse-sync-responds-with-app = Синхронизировать отклики с приложением
autoparse-sync-queued = Синхронизация запущена — результат придёт в этот чат.
autoparse-sync-empty = На hh.ru нет откликов в списке (или список пуст).
autoparse-sync-done = Синхронизация: <b>{ $inserted }</b> новых, { $skipped } уже в приложении, <b>{ $total }</b> уникальных вакансий (по списку откликов). Счётчик «Все» на hh.ru может быть больше: там считаются все отклики (в т.ч. повторы и строки без ссылки на вакансию). ❤️ В лайках этой компании (лента): <b>{ $liked_in_feed }</b> вакансий. Подтянуто с hh.ru в компанию: <b>{ $vacancies_imported }</b>.
autoparse-sync-error-no-session = Нет сохранённой браузерной сессии hh.ru. Обновите storage (см. README).
...
autoparse-keyword-check-enable = ✅ Включить проверку ключевых слов
autoparse-keyword-check-disable = ⏸️ Выключить проверку ключевых слов
autoparse-toggle-keyword-check-enabled = ✅ Проверка ключевых слов включена
autoparse-toggle-keyword-check-disabled = ⏸️ Проверка ключевых слов выключена
...
autoparse-settings-user-name = 👤 Имя для писем
autoparse-enter-user-name = Введите имя, которое будет использоваться в сопроводительных письмах (например, Иван Петров):
autoparse-settings-about-me = 📝 Несколько слов о себе
autoparse-enter-about-me = Кратко опишите себя для сопроводительных писем (например: Fullstack-разработчик с 5 годами коммерческого опыта построения высоконагруженных систем.):
...
autoparse-btn-reset-likes = 🔄 Сбросить лайки
autoparse-btn-reset-dislikes = 🔄 Сбросить дизлайки
autoparse-btn-rebuild-pool = ♻️ Пересобрать пул вакансий
autoparse-confirm-reset-likes = Очистить все лайки только для этого автопарсера? Обновятся сессии ленты по этой компании.
autoparse-confirm-reset-dislikes = Очистить все дизлайки только для этого автопарсера? Обновятся сессии ленты по этой компании.
autoparse-confirm-rebuild-pool = Пересобрать пул вакансий этого автопарсера? Это очистит сохранённые вакансии, сессии ленты, лайки и дизлайки по этой компании, но не запустит новый прогон.
autoparse-reset-likes-empty = Нет лайков для сброса.
autoparse-reset-dislikes-empty = Нет дизлайков для сброса.
autoparse-reset-likes-done = Лайки очищены.
autoparse-reset-dislikes-done = Дизлайки очищены.
autoparse-rebuild-pool-started = Пул вакансий очищен. Новый прогон автопарсинга не запускался.
autoparse-btn-update-compat-unseen = 🔄 Обновить совместимость непросмотренных вакансий
autoparse-btn-view-feed-below-compat = 📋 Лента вакансий ниже порога совместимости
autoparse-update-compat-started = ✅ Обновление совместимости запущено.
autoparse-update-compat-no-tech-stack = Добавьте стек в настройках автопарсинга.
autoparse-feed-below-compat-empty = Нет вакансий ниже вашего порога совместимости.
autoparse-feed-below-compat-title = 📋 Вакансии ниже порога совместимости
autoparse-update-compat-completed = ✅ Обновлена совместимость для { $count } вакансий.
autoparse-update-compat-timeout = Задача прервана по таймауту. Обновлено { $count } вакансий. Запустите снова для продолжения.
...
autoparse-settings-target-count = 📊 Вакансий за запуск
autoparse-settings-target-count-choose = Сколько вакансий обрабатывать за один запуск:
autoparse-settings-cover-letter-style = ✉️ Стиль сопроводительного письма
autoparse-cover-letter-style-current = Текущий: { $style }
autoparse-cover-letter-style-choose = Выберите стиль:
autoparse-cover-letter-style-professional = Профессиональный
autoparse-cover-letter-style-friendly = Дружелюбный
autoparse-cover-letter-style-concise = Краткий
autoparse-cover-letter-style-detailed = Подробный
autoparse-cover-letter-style-custom = Свой вариант
autoparse-enter-cover-letter-style = Введите описание стиля (например: формальный, креативный):
...
feed-btn-create-cover-letter = ✉️ Создать сопроводительное письмо
feed-btn-regenerate-cover-letter = 🔄 Перегенерировать
feed-btn-show-later = 🔄 Показать позже
feed-btn-stop = ⏹ Остановить
feed-content-truncated = <i>… обрезано</i>
...
feed-results-ui-applies-header = Отклики на HH (браузер)
feed-results-ui-apply-ok = ✅ { $title }
feed-results-ui-apply-employer-questions = ❓ { $title } — вопросы работодателя (завершите на hh.ru)
...
btn-iv-employer-questions = 💬 Вопросы работодателя
btn-iv-employer-qa-regenerate = 🔄 Сгенерировать заново
btn-iv-employer-qa-new = ➕ Ответить на новый вопрос
iv-employer-qa-title = Вопросы работодателя и ответы
iv-employer-qa-empty = Пока нет сохранённых вопросов. Нажмите «Ответить на новый вопрос».
iv-employer-qa-send-question = Отправьте текст вопроса от работодателя одним сообщением (как в письме или на hh.ru).
iv-employer-qa-generating = ⏳ Готовлю ответ по вашему опыту и вакансии…
iv-employer-qa-result-header = Ответ на вопрос работодателя
iv-employer-qa-label-q = Вопрос:
iv-employer-qa-label-a = Черновик ответа:
iv-employer-qa-ai-empty = Не удалось получить текст ответа. Попробуйте ещё раз позже.
iv-employer-qa-failed = Не удалось сгенерировать ответ. Попробуйте позже.
iv-employer-qa-disabled = Ответы на вопросы работодателя отключены в настройках приложения. Попросите администратора включить задачу.
iv-employer-qa-circuit = Слишком много неудач подряд; попробуйте через несколько минут.
...
btn-notes-start = Начать запись
btn-notes-stop = Остановить запись
btn-notes-edit = Редактировать заметку
btn-notes-delete = Удалить заметку
btn-notes-output-all = Вывести все заметки
btn-notes-prev = ◀ Назад
btn-notes-next = Вперёд ▶
iv-notes-page = Стр. { $current }/{ $total }
iv-notes-enter-number-edit = Введите номер заметки для редактирования (1, 2, 3...):
iv-notes-enter-number-delete = Введите номер заметки для удаления (1, 2, 3...):
iv-notes-enter-new-content = Введите новый текст для заметки { $n }:
iv-notes-added = Заметка добавлена.
iv-notes-deleted = Заметка удалена.
iv-notes-updated = Заметка обновлена.
iv-notes-invalid-number = Неверный номер. Введите 1, 2, 3...
iv-notes-noting-hint = Отправляйте сообщения, чтобы добавлять заметки. Нажмите «Остановить запись», когда закончите. Или введите /stop_notes для остановки.
iv-notes-stopped = Запись остановлена.
...
progress-group-line = <i>Шаг { $current } из { $total }{ $part2 }</i>
progress-step-negotiations-sync = 🔄 Синхронизация откликов (HH)
progress-step-autorespond-applications = 📨 Отклики и ответы
progress-step-autoparse-pipeline = 🌐 Автопарсинг
progress-step-parsing-pipeline = 🔎 Парсинг
progress-step-autorespond-pipeline = 📨 Автоответ
progress-pipeline-manual-title = Автопарсинг → Автоответ
progress-taskgroup-title = Группа задач
progress-taskgroup-macro-bar = Этапы группы
progress-taskgroup-detail-bar = Текущий этап
progress-taskgroup-nested-header = Подэтапы
progress-generic-working = Выполняется…
progress-processing = ⏳ Обработка...
progress-group-kind-autoparse-short = Автопарсинг
progress-group-kind-autorespond-short = Автоответ
progress-group-kind-parsing-short = Парсинг
progress-summary-parsed = 🔎 Спарсено вакансий: { $count }
progress-summary-autoparse-found = 🌐 Найдено новых вакансий: { $count }
progress-summary-synced = 🔄 Синхронизировано откликов: { $count }
progress-summary-responded = 📨 Обработано откликов: { $count }
progress-summary-errors = ❌ Ошибок: { $count }
progress-summary-tests = 🧪 Вакансий с тестом: { $count }
negotiations-progress-title = 🔄 Синхронизация откликов
...
progress-btn-task-title-fallback = Задача
progress-btn-try-refresh = 🔄
progress-refresh-title-hint = Прогресс по этой задаче
progress-refresh-running = Задача ещё выполняется на воркере.
progress-refresh-restarted = Задача перезапущена с контрольной точки.
progress-refresh-nothing = Нечего возобновлять (нет контрольной точки или она пуста).
progress-refresh-no-resume = Нельзя возобновить: в контрольной точке нет параметров. Запустите автоответ снова.
progress-refresh-cancelled = Этот запуск отменён.
progress-refresh-error = Не удалось перезапустить задачу. Попробуйте позже.
progress-refresh-parse-failed = Не удалось загрузить компанию парсинга для этой задачи.
progress-refresh-autoparse-failed = Не удалось загрузить компанию автопарсинга для этой задачи.
progress-refresh-unsupported = Обновление для этого типа задачи недоступно.
...
iqa-btn-add-to-interview = ➕ Добавить в интервью
...
vs-deleted = Удалено.
vs-ai-format-retry =
    Переделай ответ: нарушен требуемый формат. Сначала полный русский текст по всем шести разделам из system prompt (с эмодзи 🔥, ⭐️, ⚠️), затем на отдельной строке «---», затем только английский перевод первого раздела. Без вступлений и без вопросов в конце.
...
prep-btn-download = 📥 Скачать
prep-btn-download-md = Скачать как MD
prep-btn-download-docs = Скачать как DOCX
prep-btn-regenerate = 🔄 Перегенерировать
prep-btn-regenerate-plan = 🔄 Перегенерировать план
prep-download-title = Скачать материал
prep-docs-generating = Генерирую документ...
prep-docs-failed = Не удалось создать документ.
prep-regenerating = Перегенерирую...
...
parsing-staleness-error = ⏱ За последние { $minutes } мин. не было прогресса. Задача остановлена. Попробуйте снова.
