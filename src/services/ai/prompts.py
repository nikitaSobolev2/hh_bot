...
_COMPATIBILITY_SCORING_GUIDE_RU = (
    ...
    "2) Предметный домен и узкие навыки (ITSM, BPMN, отраслевая экспертиза, узкие платформы): "
    "если их нет в профиле кандидата — сильно снижай оценку; "
    "общие инструменты это не компенсируют.\n"
    "3) Пересечение стека усиливает балл только когда роль и домен уже близки; не прибавляй баллы "
    "за каждое случайное совпадение ключевого слова.\n\n"
    "[ШКАЛА]\n"
...
def build_vacancy_summary_system_prompt() -> str:
    ...
    return (
        ...
        "без вопросов. Начинай сразу с первого абзаца текста «О себе».\n\n"
        "Обычный текст без markdown. Только эмодзи для структурирования. "
        "Каждый раздел с новой строки.\n\n"
        "[АНГЛИЙСКАЯ ВЕРСИЯ]\n"
        "В конце текста добавь «---» и продублируй первый раздел («Кто я») на английском. "
        "Переведи вступление естественно для англоязычного резюме.\n\n"
        f"{_STRICT_OUTPUT_PROHIBITION}\n\n"
        f"{_ANTI_INJECTION}"
    )
...
def build_vacancy_summary_user_content(
    ...
    parts.append(
        "Напиши профессиональный текст 'О себе' на основе этих данных. "
        "В конце добавь «---» и продублируй первый раздел (вступление) на английском."
    )
    return "\n\n".join(parts)
...
def build_cover_letter_system_prompt(style: str) -> str:
    ...
    style_guidance = {
        ...
        "detailed": (
            "Стиль: чуть подробнее, но в рамках 2 абзацев. Ключевые достижения."
        ),
    ...
    return (
        ...
        "для HR-рекрутеров.\n\n"
        "[ЗАДАЧА]\n"
        "Напиши сопроводительное письмо в 2 абзаца (100–150 слов). "
        "HR должен прочитать за 30 секунд. Максимально привлекательно и по делу.\n\n"
        f"[СТИЛЬ]\n{style_guidance}\n\n"
        "[СТРУКТУРА — СТРОГО СЛЕДУЙ ЭТОМУ ШАБЛОНУ]\n"
        "1. Первое предложение: [Имя] — [роль] на стыке [ключевые технологии] "
        "с [архитектура/специализация]. Используй [НЕСКОЛЬКО СЛОВ О СЕБЕ] если есть, "
        "иначе из опыта. Без приветствий — сразу суть.\n"
        "2. Второе предложение: Конкретные достижения (что делал, какие технологии внедрял, "
        "какой результат — пропускная способность, latency, масштабирование и т.п.).\n"
        "3. Третье предложение: «Опыт релевантен для [задачи/продукта из вакансии]» — "
        "явная связь с вакансией.\n"
        "4. Четвёртое предложение: «В вашем стеке — [технологии из вакансии] — "
        "работал со всем этим и внедрял [SOLID/тесты/документация и т.п.].»\n"
        "5. Пятое предложение: «Хочу [конкретные задачи из вакансии] в вашей команде.» — "
        "мотивация, завязанная на обязанности вакансии.\n\n"
        "[ИДЕАЛЬНЫЙ ПРИМЕР]\n"
        "«Никита — Fullstack-разработчик на стыке Python и Node.js с архитектурой микросервисов. "
        "Разрабатывал высоконагруженные системы обработки заказов, внедрял Kafka и Docker, "
        "что повысило пропускную способность и снизило latency. Опыт релевантен для сложных задач "
        "iGaming и микросервисной архитектуры. В вашем стеке — NestJS, TypeScript, PostgreSQL, "
        "Docker — работал со всем этим и внедрял SOLID. Хочу автоматизировать процессы и решать "
        "сложные интеграционные задачи в вашей команде.»\n\n"
        "[ЗАПРЕЩЕНО]\n"
        "- «Добрый день», «Уважаемая команда», «Уважаемый HR», «С уважением» и подобные.\n"
        "- Письмо начинается сразу с «[Имя] — [роль]», без обращения.\n"
        "- Выдумывать цифры, метрики, факты. Только данные кандидата.\n\n"
        "[ПРАВИЛА]\n"
        "- Пиши от первого лица. Используй имя кандидата из блока [ИМЯ КАНДИДАТА].\n"
...
def build_cover_letter_user_content(
    ...
    instruction = (
        "Напиши сопроводительное письмо в 2 абзаца (100–150 слов). "
        "Без обращения и подписи. Максимально привлекательно для HR."
    )
    if about_me and about_me.strip():
        instruction += " Используй [НЕСКОЛЬКО СЛОВ О СЕБЕ] для позиционирования в первом абзаце."
    parts.extend(
        [
