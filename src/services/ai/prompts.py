...
def build_vacancy_summary_system_prompt() -> str:
    ...
    return (
        ...
        "[АНГЛИЙСКАЯ ВЕРСИЯ]\n"
        "В конце текста добавь разделитель «---» и продублируй первый раздел («Кто я») на английском. "
        "Переведи вступление (специальность, опыт, ключевые технологии) естественно для англоязычного резюме.\n\n"
        f"{_STRICT_OUTPUT_PROHIBITION}\n\n"
        f"{_ANTI_INJECTION}"
    )
...
def build_cover_letter_system_prompt(style: str) -> str:
    ...
    return (
        ...
        "для HR-рекрутеров.\n\n"
        "[ЗАДАЧА]\n"
        "Напиши сопроводительное письмо в 2 абзаца (80–120 слов). "
        "HR должен прочитать за 30 секунд. Максимально привлекательно и по делу.\n\n"
        f"[СТИЛЬ]\n{style_guidance}\n\n"
        "[СТРУКТУРА — СТРОГО 2 АБЗАЦА]\n"
        "Абзац 1: Краткое представление (имя из данных), интерес к вакансии/компании, "
        "ключевой релевантный опыт (1–2 предложения).\n"
        "Абзац 2: Конкретные достижения, связь с требованиями вакансии, "
        "призыв к действию — встроить естественно, без формального «Готов обсудить».\n\n"
        "[ЗАПРЕЩЕНО]\n"
