...
def build_achievement_generation_system_prompt() -> str:
    ...
    return (
        ...
        "Пример: «Перевел сервис биллинга на микросервис с использованием брокера "
        "для обработки платежей, обеспечив масштабируемость и гарантию проведения платежа.»\n"
...
def build_per_company_key_phrases_prompt(
    ...
    return (
        ...
        "[ОДИНАКОВЫЕ НАЗВАНИЯ КОМПАНИЙ]\n"
        "Если в опыте работы несколько записей с одинаковым названием компании — "
        "это карьерный рост в одной компании (например, Junior → Senior). "
        "Генерируй фразы так, чтобы была видна прогрессия: более ранняя роль — "
        "более простые/исполнительские задачи, более поздняя — более сложные, "
        "архитектурные или менторские. Связывай технологии и контекст между ролями.\n\n"
        "Основные ключевые слова (каждое должно появиться хотя бы раз): "
        f"[{main_joined}].\n"
        f"Дополнительные ключевые слова (используй уместные): "
        f"[{secondary_joined}].\n\n"
        "[СТЕК И КОНТЕКСТ]\n"
        "- Используй ТОЛЬКО технологии из стека кандидата для данной компании. "
        "Не выдумывай технологии, которых нет в стеке.\n"
        "- Каждая фраза — конкретная задача (например: «Интегрировал X для Y», "
        "«Настроил Z, включая A и B»), а не общие обязанности.\n"
        "- Избегай абстрактных buzzword'ов («code review», «design patterns», «SOLID») "
        "как главный фокус — без контекста они малоценны.\n"
        "- Учитывай уровень роли: Junior/Middle — фокус на реализации; "
        "Senior — можно включать архитектуру, менторство, владение pipeline.\n\n"
        "[ПЛОХИЕ ПРИМЕРЫ — ИЗБЕГАТЬ]\n"
        "- «Разрабатывал backend‑модули на Symfony, используя Doctrine ORM и "
        "SQL‑запросы к PostgreSQL.» — PostgreSQL не в стеке кандидата для этой компании.\n"
        "- «Проводил code review и применял design patterns в JavaScript и PHP.» — "
        "слишком абстрактно, нет конкретной задачи.\n"
        "- «Осуществлял контейнеризацию с Docker и автоматизировал деплой через "
        "GitLab CI/CD.» — просто перечисление инструментов без контекста и глубины.\n\n"
        "[СРЕДНИЕ ПРИМЕРЫ — ЛУЧШЕ, НО НЕДОСТАТОЧНО]\n"
        "- «Создавал микросервис на Node.js с TypeScript и NestJS, организуя обмен "
        "сообщениями через RabbitMQ.» — RabbitMQ не в стеке.\n"
        "- «Разрабатывал REST API на Django с использованием DRF.» — "
        "Django не в стеке (у кандидата FastAPI, Laravel).\n"
        "- «Выполнял code review, наставлял младших разработчиков и поддерживал "
        "Linux‑серверы под Nginx.» — Nginx не в стеке.\n"
        "Причина: хорошая структура, но выдуманные технологии.\n\n"
        "[ОТЛИЧНЫЕ ПРИМЕРЫ — ЦЕЛЕВОЙ УРОВЕНЬ]\n"
        "- «Настраивал CI/CD pipeline в GitLab, включая Docker Compose и "
        "автоматическое unit‑testing.» — только стек кандидата, конкретная задача.\n"
        "- «Интегрировал Elasticsearch для полнотекстового поиска по продуктовым "
        "данным.» — конкретная задача + контекст (CRM/ERP).\n"
        "- «Интегрировал ClickHouse в аналитический pipeline, позволяя быстро "
        "агрегировать данные о доставках.» — стек + бизнес-контекст.\n"
        "- «Поддерживал микросервисы на FastAPI и Laravel, развернутые в "
        "Kubernetes с Helm‑чартами.» — технологии из стека, конкретная зона.\n\n"
        f"[ОПЫТ РАБОТЫ КАНДИДАТА]\n"
        f"{companies_block}\n\n"
        "[ФОРМАТ ВЫВОДА]\n"
        "Выводи результат строго по формату:\n"
...
def build_work_experience_achievements_system_prompt() -> str:
    ...
    return (
        ...
        "«Перевел сервис биллинга на микросервис с использованием брокера "
        "для обработки платежей, обеспечив масштабируемость и гарантию проведения платежа.»\n"
