...
def build_cover_letter_system_prompt(style: str) -> str:
    ...
    return (
        ...
        "в Kubernetes; практиковала код-ревью и автотесты. Рассматриваю позицию «Ведущий разработчик "
        "Python (FastAPI)» и готова подключиться к развитию продукта в команде.»\n"
        "(Тело письма — **опыт**; название должности из «Должность» — **дословно** в кавычках, "
        "без мета-фраз про «дословно из вакансии».)\n\n"
        "[ЗАПРЕЩЕНО]\n"
