...
def build_cover_letter_system_prompt(style: str) -> str:
    ...
    return (
        ...
        "для HR-рекрутеров.\n\n"
        "[ЗАДАЧА]\n"
        "Напиши одно связное сопроводительное письмо — один абзац без подзаголовков и списков. "
        "Объём примерно 80–120 слов; HR читает за ~30 секунд. Цель: кратко передать профиль, "
        "релевантный опыт и уместность кандидата именно под эту вакансию.\n\n"
        f"[СТИЛЬ]\n{style_guidance}\n\n"
        "[СОДЕРЖАНИЕ — ОРИЕНТИРЫ, НЕ ЖЁСТКИЙ ШАБЛОН]\n"
        "- Начни с имени из [ИМЯ КАНДИДАТА] и короткого позиционирования (роль, ключевой стек, "
        "домен или архитектура). При наличии [НЕСКОЛЬКО СЛОВ О СЕБЕ] — вплети это в начало.\n"
        "- 1–2 предложения: что делал на практике (технологии, зоны ответственности), "
        "качественно: «существенно повысил устойчивость», «стабилизировал», "
        "«масштабировал без простоев» — без цифр.\n"
        "- Явно свяжи опыт с этой вакансией: **название должности укажи дословно**, "
        "как в строке «Должность» в блоке [ВАКАНСИЯ] пользователя (не перефразируй и не подменяй "
        "уровень: не пиши «Senior …», если в вакансии другая формулировка). "
        "Кратко — стек или задачи из описания вакансии.\n"
        "- Заверши мотивацией в деловом тоне (интерес к роли и задачам из описания).\n\n"
        ...
        "(FastAPI)» из вашего описания совпадает с моим стеком и задачами; готова подключиться к "
        "развитию продукта в вашей команде.»\n"
        "(Название в кавычках после слова «Позиция» — **подставь дословно строку «Должность»** "
        "из блока [ВАКАНСИЯ] пользователя, не сокращай и не переименовывай.)\n\n"
        "[ЗАПРЕЩЕНО]\n"
...
def build_cover_letter_user_content(
    ...
    instruction = (
        "Напиши сопроводительное письмо: один абзац (примерно 80–120 слов), без обращения "
        "и подписи, без процентов и других «сырых» цифр в тексте письма. "
        "Название позиции в письме возьми **дословно** из строки "
        "«Название вакансии для текста письма» ниже (это та же строка, что «Должность»). "
        "Максимально привлекательно для HR."
    )
    if about_me and about_me.strip():
        instruction += " Используй [НЕСКОЛЬКО СЛОВ О СЕБЕ] в начале позиционирования."
    parts.extend(
        [
