...
def build_cover_letter_system_prompt(style: str) -> str:
    ...
    return (
        ...
        "- **Название должности** из строки «Должность» в [ВАКАНСИЯ] вплети в текст естественно "
        "(например, в кавычках «…» в середине или конце абзаца). Не перефразируй и не подменяй уровень.\n"
        "- В конце — коротко (одно предложение) мотивация в деловом тоне, без пересказа описания "
        "вакансии абзацем.\n\n"
        "[ЛИЦО И ГРАММАТИКА — ОТ ПЕРВОГО ЛИЦА]\n"
        "- Пиши **от первого лица**, как сам кандидат обращается к работодателю: «я», «мне», "
        "«мой/моя/мои»; глаголы в 1-м лице: «рассматриваю», «имею», «готов/готова», "
        "«занимался/занималась», «проектировал/проектировала».\n"
        "- Допустимо начать с «Имя — …», но дальше **без** резюме в 3-м лице: не «Имеет опыт», "
        "«Рассматривает позицию», «обладает», «является специалистом».\n"
        "- Запрещены формулировки HR/резюме о кандидате в 3-м лице: «кандидат», «он/она», "
        "«специалист имеет», «рассматривает данную позицию».\n\n"
        ...
        "без чисел.\n\n"
        "[ИДЕАЛЬНЫЙ ПРИМЕР]\n"
        "«Никита Соболев — fullstack-разработчик с фокусом на Python; занимался созданием "
        "и поддержкой веб-приложений на Django, проектировал REST API, работал с PostgreSQL "
        "и писал сложные SQL-запросы. Использовал React и TypeScript на клиенте, применял Celery, "
        "RabbitMQ и Redis для асинхронных задач, выкладывал сервисы в Docker и Kubernetes. "
        "Рассматриваю позицию «Django разработчик» и готов усилить команду Pentestit экспертизой "
        "в безопасной веб-разработке.»\n"
        "(Тело письма — **опыт от первого лица**; название должности из «Должность» — "
        "**дословно** в кавычках, без мета-фраз про «дословно из вакансии».)\n\n"
        "[ЗАПРЕЩЕНО]\n"
        ...
        "- Описание кандидата в 3-м лице: «Имеет опыт», «Рассматривает позицию», «обладает», "
        "«является специалистом», «кандидат».\n"
        "- Мета-фразы и подписи полей промпта: «Дословно из вакансии», «Название вакансии для текста "
        "письма», «Должность:», «из вашей вакансии» — только живой текст письма.\n\n"
        "[ПРАВИЛА]\n"
        "- Тон: нейтрально-профессиональный, **от первого лица**; финальная мотивация — "
        "«Рассматриваю позицию…», «Готов/готова…», не «Рассматривает» / «Имеет».\n"
...
def build_cover_letter_user_content(
    ...
    instruction = (
        "Напиши **один абзац** сопроводительного письма **от первого лица** (я, мне, мой; "
        "«рассматриваю», «имею», «готов/готова» — не «рассматривает», «имеет»): это текст "
        "**про опыт кандидата**, подобранный и сформулированный так, чтобы он показывал "
        "соответствие **технологиям и требованиям** вакансии (см. описание выше). "
        "Без обращения и подписи, без «сырых» цифр. "
        "Название должности из строки «Должность» вплети **дословно** в текст (в кавычках «…»), "
        "без мета-фраз вроде «Дословно из вакансии». Не пересказывай описание вакансии; "
        "пиши про опыт кандидата."
    )
    if about_me and about_me.strip():
