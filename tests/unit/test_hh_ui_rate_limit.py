...
@pytest.fixture
def mock_rate_limit():
    with patch("src.services.hh_ui.rate_limit.get_hh_ui_apply_max_per_day_effective", return_value=2):
        with patch("src.services.hh_ui.rate_limit.settings") as s:
            s.redis_url = "redis://localhost:9/0"
            yield s


def test_rate_limit_allows_within_limit(mock_rate_limit) -> None:
    r = MagicMock()
...
def test_rate_limit_blocks_over_limit(mock_rate_limit) -> None:
    r = MagicMock()
...
def test_current_ui_apply_count_reads_redis(mock_rate_limit) -> None:
    r = MagicMock()
...
def test_current_ui_apply_count_missing_key(mock_rate_limit) -> None:
    r = MagicMock()
...
def test_remaining_ui_apply_slots_unlimited(mock_rate_limit) -> None:
    with patch("src.services.hh_ui.rate_limit.get_hh_ui_apply_max_per_day_effective", return_value=0):
        assert remaining_ui_apply_slots_sync(1) is None


def test_remaining_ui_apply_slots(mock_rate_limit) -> None:
    r = MagicMock()
