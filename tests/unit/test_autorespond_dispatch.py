...
async def test_dispatch_filters_capped_pre_skipped_and_seeds_ready_zset(
    ...
    assert pump_kick.call_count == 1
    assert pump_delay.call_count == 0
    assert tick_mock.await_count == 1  # one pre-skip tick
    assert len(envelope_calls) == 1
    assert envelope_calls[0]["resume_envelope"]["user_id"] == 7
    assert envelope_calls[0]["ready_queued"] == 1


@pytest.mark.asyncio
async def test_dispatch_kicks_pump_after_first_resume_without_waiting_for_batch(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Each resolved resume enqueues pregen immediately; pump starts on the first unit."""
    session, session_factory = _make_session_factory()
    ...
    monkeypatch.setattr(
        "src.worker.tasks.autorespond._start_or_update_progress_bar",
        AsyncMock(return_value=(progress, "autorespond:11:tid", MagicMock(), 0, False)),
    )
    for fn in (
        "clear_autorespond_done_counter",
        "clear_autorespond_failed_counter",
        ...
        monkeypatch.setattr(f"src.services.autorespond_progress.{fn}", AsyncMock())
    for fn in (
        "clear_hh_ui_batch_checkpoint_sync",
        "clear_hh_ui_resume_envelope_sync",
    ):
    ...
    assert pregen_delay.call_count == 2
    assert pump_kick.call_count == 1
    assert len(envelope_calls) == 2
    assert envelope_calls[0]["ready_queued"] == 1
    assert envelope_calls[1]["ready_queued"] == 1
    assert envelope_calls[1]["total_work_units"] == 0
