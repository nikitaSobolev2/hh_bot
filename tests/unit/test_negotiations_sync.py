...
async def test_fetch_merged_vac_dicts_abort_remaining_requests_on_captcha():
    ...
    async def fake_parse_vacancy_page(self, client, url, **kwargs):
    ...
    with (
        ...
        pytest.raises(HHCaptchaRequiredError),
    ):
...
async def test_sync_negotiations_returns_error_on_captcha_abort():
    ...
    with (
        ...
        patch(
            "src.worker.tasks.negotiations_sync.fetch_merged_vac_dicts_for_hh_ids",
            new=AsyncMock(
